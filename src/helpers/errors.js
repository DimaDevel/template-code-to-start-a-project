...
function getErrorObject(systemCode, status = 500, error = null) {
  ...
  if (error && error.name === 'ValidationError' && error.errors) {
    ...
    // generate message from errorObject higher in code to string like  
    // Sorry, but this prop1, prop2, ... is already in use || prop1, prop2,
    // ... is required || prop1, prop2, ... errorMessage;
    // And joining all of this messages in One
    errorObject.message = Object.entries(errors)
      .map(([key, value]) => (key.includes('already')
